#ifndef GLUON_WRAPPERS_POINTER_HPP_
#define GLUON_WRAPPERS_POINTER_HPP_
...
#define SAFE_PTR_NULL_HANDLE_CHECK(handle, ...)         \
    if (handle) {                                       \
        return __VA_ARGS__                              \
    }                                                   \
    throw Gluon::Exceptions::NullHandleException()

#define SAFE_PTR_UNITY_NULL_HANDLE_CHECK(...)           \
    if (isAlive()) {                                    \
        return __VA_ARGS__;                             \
    }                                                   \
    throw Gluon::Exceptions::NullHandleException()
...
namespace Gluon {
    ...
    struct Counter {
        ...
        static void add(void* addr) {
            ...
            if (const auto itr = addrRefCount.find(addr); itr != addrRefCount.end()) {
                ++itr->second;
            } else {
        ...
        static void remove(void *addr) {
            std::unique_lock lock(mutex);
            if (const auto itr = addrRefCount.find(addr); itr != addrRefCount.end() && itr->second > 1) {
                --itr->second;
    ...
    struct CountPointer {
        ...
        constexpr T *__internal_get() const noexcept {
            return ptr;
        }
    ...
    struct SafePtr {
        ...
        explicit SafePtr(T* wrappableInstance) : internalHandle(SafePointerWrapper::New(wrappableInstance)) {}
        /// @brief Construct a SafePtr<T> with the provided wrapper
        explicit SafePtr(T& wrappableInstance)
            requires(Gluon::TypeConcepts::HasIl2CppConversion<T>)
        ...
        inline void clear() {
            ...
            if (internalHandle.count() <= 1) {
                Gluon::Il2CppFunctions::initialise();
                Gluon::Il2CppFunctions::gc_free_fixed(internalHandle.__internal_get());
            }
    ...
    private:
        ...
        struct SafePointerWrapper {
            static SafePointerWrapper* New(T* instance) {
                ...
                auto* wrapper = static_cast<SafePointerWrapper*>(Gluon::Il2CppFunctions::gc_alloc_fixed(sz));
    ...
    public:
        ...
        explicit SafePtrUnity(T* wrappableInstance) : Parent(wrappableInstance) {}
        explicit SafePtrUnity(T& wrappableInstance) : Parent(wrappableInstance) {}
        explicit SafePtrUnity(Parent&& p) : Parent(p) {}
        explicit SafePtrUnity(Parent const& p) : Parent(p) {}
    ...
    struct FunctionWrapper<R (*)(TArgs...)> final : AbstractFunction<R(void*, TArgs...)> {
        ...
        R (*held)(TArgs...);
        template <class F>
        explicit FunctionWrapper(F&& f) : held(f) {}
    ...
    template <typename R, typename T, typename... TArgs>
    struct FunctionWrapper<R (T::*)(TArgs...)>final : AbstractFunction<R(void*, TArgs...)> {
        void* instance() const override {
            return _instance;
        }
        void* ptr() const override {
    ...
    template <typename R, typename... TArgs>
    struct FunctionWrapper<std::function<R(TArgs...)>>final : AbstractFunction<R(void*, TArgs...)> {
        ...
        explicit FunctionWrapper(std::function<R(TArgs...)> const& f) : held(f), handle(const_cast<void*>(reinterpret_cast<const void*>(&f))) {}
        R operator()(TArgs... args) const noexcept override {
            if constexpr (std::is_same_v<R, void>) {
                held(args...);
            } else {
    ...
    public:
        explicit ThinVirtualLayer(R (*ptr)(TArgs...)) : func(new FunctionWrapper<R (*)(TArgs...)>(ptr)) {}
        template <class F, typename Q>
        ThinVirtualLayer(F&& f, Q* inst) : func(new FunctionWrapper<R (Q::*)(TArgs...)>(f, inst)) {}
        template <class F>
        explicit ThinVirtualLayer(F&& f) : func(new FunctionWrapper<std::function<R(TArgs...)>>(f)) {}
    ...
    public:
        void invoke(TArgs... args) const {
            ...
            for (auto cbs = callbacks; auto& callback : cbs) {
                callback(args...);
            }
#else
        ...
        void removeCallback(void (T::*callback)(TArgs...)) {
            ...
            for (auto itr = callbacks.begin(); itr != callbacks.end();) {
                ...
                if (dat d{ .wrapper = callback }; itr->ptr() == d.data) {
...
#endif // GLUON_WRAPPERS_POINTER_HPP_